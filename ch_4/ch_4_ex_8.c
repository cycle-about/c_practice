...
Presume: use the only-one-char-buffered setup as in 4-8

Try running without the clause to not push back EOF, see if any effects
	-> seems this would mean input ended with a number, which seems invalid reverse
	Polish notation anyway?

Issue: I do not know how to get EOF as last char from terminal input to test this, 
	ctrl+d does nothing and ctrl+c ends program, \n computes
...
int getop(char s[]) {
	...
	// original implementation: if first non-digit char WAS eof, it is never read
	//if (c != EOF)    // if first non-digit char was anything but EOF, still needs to be read
	ungetch(c); 	
