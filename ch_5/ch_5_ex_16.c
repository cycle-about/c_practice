...
Idea: Use the copy-of-modified-string setup from 5-15, and have the copy omit all
chars not meeting that criteria, and then compare pointers to those copies

Note: this means the 'regular' qsort ALSO needs the 'array copy' setup, so likely
make into only a single qsort method for all cases again

First version though: add this to the 'nocase' version of function
...
void qsort_nocase(int reverse, int nocase, int alnum, void *lineptr[], int left, int right, int (*comp)(void *, void *));
...
int main(int argc, char *argv[]) {
	...
	reverse = checkflag("-r", argc, argv);	// descending order
	nocase = checkflag("-f", argc, argv);  	// ignores capitalization
	alnum = checkflag("-d", argc, argv); 	// compares only letters, numbers, and ' '
	...
	if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
		...
		// MESSY FOR NOW
		if (!numeric && (nocase || alnum)) {  // function version for lex with nocase or alnum
			printf("\nUsing qsort_nocase\n");
			qsort_nocase(reverse, nocase, alnum, (void **) lineptr, 0, nlines-1, (int (*)(void*,void*))(numeric ? numcmp : strcmp));
		} else {  // function for numeric or case sensitive
...
// version of qsort for lexicographic sorting when ignoring case, or 'directory order'
void qsort_nocase(int reverse, int nocase, int alnum, void *v[], int left, int right, int (*comp)(void *, void *)) {
	int i, last, compared;
	void swap(void *v[], int, int);
	char leftlower[100];  	// array to store processed version of string
	char ilower[100];
	...
	// TODO handle that either or both of these might run
	if (nocase) 
